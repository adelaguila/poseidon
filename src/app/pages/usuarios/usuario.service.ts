...
export class UsuarioService {
  ...
  actualizarPerfil(data: { email: string; fullName: string }) {
    return this.http.patch(`${api_url}/auth/`, data, { context: checkToken()});
  }
  ...
  getUsuarios(parametros?: any, page?: number, filtros?: any): Observable<any> {
    const params = new HttpParams()
      .set('page', page)
  ...
  cambiarEstado(usuario: Usuario){
    return this.http.patch(`${api_url}/auth/cambiar-estado/${usuario.id}`, '', { context: checkToken()});
  }
  ...
  cambiarRole(usuario: Usuario) {
    ...
    return this.http.patch(`${api_url}/auth/cambiar-role/${id}`, usuario, { context: checkToken()});
  }
}
